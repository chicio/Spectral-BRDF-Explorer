...
float[samples] calculateSpectrum(DirectionalSpectralLight light,
    ...
    float specularComponent = blinnPhongModelComponent * surfaceMaterial.specularPercentage;
...
void main() {
    ...
    float blinnPhongSpecularComponent = pow(max(0.0, dot(h, viewDirection)), surfaceMaterial.sh);
