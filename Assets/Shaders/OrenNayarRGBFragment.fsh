...
void main() {
    ...
    if(textureActive == 0) {
        ...
        diffuse =  surfaceMaterial.kd * light.color * orenNayar(lightDirection,
                                                                viewDirection,
                                                                normalize(normalInterp)) *  cosTheta;
    } else {
            
        //Lighting using texture.
        diffuse = surfaceMaterial.kd * texture(textureSampler,
                                               textureCoordinate) * orenNayar(lightDirection,
                                                                              viewDirection,
                                                                              normalInterp) * cosTheta;
    }
